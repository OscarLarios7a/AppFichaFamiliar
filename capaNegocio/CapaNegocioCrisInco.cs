...
{
    public class capaNegocioCrisInco
    {
        ...
        public void CargarGrid(DataGridView grid)
        {
            ...
            try
            {
                ...
                string query = string.Format("SELECT idFamilia,idIntegrante,curpTutora,nombreCTutora,CR_CRIS_2021,FECHA_ENCUESTA_CRIS_2021,CR_INCO_2021,FECHA_ENTREGA_INCO_2021,CR_CRIS_2020,FECHA_ENCUESTA_CRIS_2020,CR_INCO_2020,FECHA_ENTREGA_INCO_2020  from cris_inco ");//creamos la consulta a la base 
        ...
        public void CargarGridBuscar(DataGridView grid, string txtBuscar)
        {
            ...
            try
            {
                ...
                string query = string.Format("SELECT idFamilia,idIntegrante,curpTutora,nombreCTutora,CR_CRIS_2021,FECHA_ENCUESTA_CRIS_2021,CR_INCO_2021,FECHA_ENTREGA_INCO_2021,CR_CRIS_2020,FECHA_ENCUESTA_CRIS_2020,CR_INCO_2020,FECHA_ENTREGA_INCO_2020  from cris_inco WHERE idFamilia LIKE '%{0}%' ", txtBuscar);//creamos la consulta a la base 
                //creamos el cmd para que se lleve el query y cargue la conexion con la DB
                MySqlCommand cmd = new MySqlCommand(query, cDConexionBd.GetConnection());
