...
namespace capaNegocio
{
    ...
    public class capaNegocioDatosTerritoriales
    {
        ...
        public void CargarGrid(DataGridView grid)
        {
            ...
            try
            {
                ...
                string query = string.Format("SELECT  idEstructuraTerritorial,idFamilia,idRegion,region,cveLocOfi,cveMun, municipio,cveLoc,localidad from estructterritorial ");//creamos la consulta a la base 
        ...
        public void CargarGridBuscar(DataGridView grid, string txtBuscar)
        {
            ...
            try
            {
                ...
                string query = string.Format("SELECT idEstructuraTerritorial,idFamilia,idRegion,region,cveLocOfi,cveMun, municipio,cveLoc,localidad from estructterritorial WHERE idFamilia LIKE '%{0}%' ", txtBuscar);//creamos la consulta a la base 
                //creamos el cmd para que se lleve el query y cargue la conexion con la DB
                MySqlCommand cmd = new MySqlCommand(query, cDConexionBd.GetConnection());
